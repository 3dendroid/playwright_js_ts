...
test.describe('Поиск элементов форм по ролям', () => {
  ...
  test('Найти поля формы по их ролям', async ({ page }) => {
    const usernameInput = page.getByText('Имя пользователя'); // TODO(student): замените на корректный локатор
  ...
  test('Найти чекбоксы по роли checkbox', async ({ page }) => {
    const newsletterCheckbox = page.getByText('Подписаться на рассылку'); // TODO(student): замените на корректный локатор
