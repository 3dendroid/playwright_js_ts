...
test.describe('Базовые тесты для getByPlaceholder()', () => {
  ...
  test('Найти поле по части placeholder', async ({ page }) => {
    const emailInput = page.getByRole('textbox', { name: 'example@mail.com' }); // TODO(student): замените на корректный локатор
...
test.describe('Сложные случаи для getByPlaceholder()', () => {
  ...
  test('Найти textarea по многострочному placeholder', async ({ page }) => {
    const textarea = page.getByRole('heading', { name: "Разнообразные placeholder'ы" }); // TODO(student): замените на корректный локатор
  ...
  test('Работа с динамическими полями', async ({ page }) => {
    const dynamicInput = page.getByText('Динамические поля появятся здесь через 1 секунду'); // TODO(student): замените на корректный локатор
